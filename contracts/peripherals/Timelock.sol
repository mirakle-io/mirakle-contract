...
contract Timelock is ITimelock {
    ...
    function initGlpManager() external onlyAdmin {
        ...
        IUSDM usdg = IUSDM(_glpManager.usdg());
    ...
    function updateUsdgSupply(uint256 usdgAmount) external onlyKeeperAndAbove {
        ...
        IUSDM(usdg).addVault(address(this));

        if (usdgAmount > balance) {
            uint256 mintAmount = usdgAmount.sub(balance);
            IUSDM(usdg).mint(glpManager, mintAmount);
        } else {
            uint256 burnAmount = balance.sub(usdgAmount);
            IUSDM(usdg).burn(glpManager, burnAmount);
        }

        IUSDM(usdg).removeVault(address(this));
    }
    ...
    function setGlpCooldownDuration(uint256 _cooldownDuration)
        external
        onlyAdmin
    {
        require(
    ...
    function redeemUsdg(
        ...
        IUSDM(usdg).mint(address(this), _amount);
